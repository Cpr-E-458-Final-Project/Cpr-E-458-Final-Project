...
public class EDF_Checker implements SchedulabilityChecker
{
	...
	{
		return new EDF_Checker().checkSchedulability(tasklist);
	}
	
	public static void detailedCheck(TaskList tasklist)
	{
		new EDF_Checker().properOutput(tasklist);
	}
	...
	public void properOutput(TaskList tasklist)
	{
		...
		if(lhs.getProcessedValue() <= rhs.getProcessedValue())
			System.out.println("The given TaskList is schedulable by the EDF algorithm.");
		else
			System.out.println("The given TaskList is not schedulable by the EDF algorithm.");
		
		if(manual_check)
		{
			System.out.println("HOWEVER, because there are deadlines shorter than the period in the given TaskList, manual checking will be required.  Now scheduling using EDF...");
			if(new EDF_Scheduler().schedule(tasklist))
				System.out.println("The given TaskList is indeed schedulable by the EDF algorithm.");
			else
				System.out.println("The given TaskList is not schedulable by the EDF algorithm.");
		}
